...
public class ABaseController {
    ...
    protected void convertFileResponse2Stream(HttpServletResponse httpServletResponse, Response response) {
        //Response.body().asInputStream() 直接获取远程响应的输入流，绕过了 Feign 的自动反序列化，确保二进制数据（如视频流）以原始形式传输，避免格式转换错误
        Response.Body body = response.body();
        try (InputStream inputStream = body.asInputStream(); ServletOutputStream outputStream = httpServletResponse.getOutputStream()) {
